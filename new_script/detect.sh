...
taskset -c 0-$((cpu_cores_per_gpu - 1)) python custom/detection.py --cam_path "$video_1" --cam-id 1 --device 0&

# Camera 1 → GPU 1
export CUDA_VISIBLE_DEVICES=1
taskset -c $((cpu_cores_per_gpu))-$(($cpu_cores_per_gpu * 2 - 1)) python custom/detection.py --cam_path "$video_2" --cam-id 2 --device 1&
